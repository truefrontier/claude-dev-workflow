...
async function validateRepository() {
  try {
    const output = execSync('gh repo view --json nameWithOwner,viewerPermission,viewerCanAdminister', { 
      encoding: 'utf8',
      stdio: 'pipe' 
    });
    ...
    return {
      ...
      canAdminister: repoInfo.viewerCanAdminister
    };
...
async function validatePermissions() {
  try {
    const output = execSync('gh repo view --json viewerPermission,viewerCanAdminister', {
      encoding: 'utf8',
      stdio: 'pipe'
    });
    
    const repoInfo = JSON.parse(output);
    const permission = repoInfo.viewerPermission;
    const canAdmin = repoInfo.viewerCanAdminister;
    
    // Check for admin or maintain permissions
    if (!canAdmin && permission !== 'ADMIN' && permission !== 'MAINTAIN') {
      const err = new Error('Repository admin or maintain access required for setup');
...
async function validateLabels() {
  try {
    ...
    const requiredLabels = [
      'needs:specify', 'needs:specify-revision',
      'needs:plan', 'needs:plan-revision',
      'needs:tasks', 'needs:tasks-revision',
      'needs:develop', 'needs:develop-revision',
      'review:specify', 'review:plan', 'review:tasks', 'review:develop',
      'error:specify', 'error:plan', 'error:tasks', 'error:develop'
    ];
