...
function extractTokenConfiguration(content) {
  ...
  if (oauthMatch) {
    ...
    // Also check for commented API key that should be preserved
    const commentMatch = content.match(/^(\s*)#\s*anthropic_api_key:\s*\$\{\{\s*secrets\.[^}]+\s*\}\}\s*$/m);

    return {
      type: 'oauth',
      secretName: secretName,
      line: `claude_code_oauth_token: \${{ secrets.${secretName} }}`,
      commentLine: commentMatch ? commentMatch[0] : null
    };
  }
...
function applyTokenConfiguration(newContent, tokenConfig) {
  ...
  if (tokenConfig.type === 'oauth') {
    // Replace active API key with OAuth token and add commented API key if user had one
    let result = newContent;

    // Replace active anthropic_api_key line with OAuth token and optional comment
    result = result.replace(
      /^(\s*)anthropic_api_key:\s*\$\{\{\s*secrets\.[^}]+\s*\}\}\s*$/gm,
      (match, indent) => {
        let replacement = `${indent}${tokenConfig.line}`;
        // If user had a commented API key line, add it before the OAuth line
        if (tokenConfig.commentLine) {
          replacement = `${indent}${tokenConfig.commentLine.trim()}\n${replacement}`;
        }
        return replacement;
      }
    );

    // If there was no active API key line to replace, add OAuth token after the 'with:' line
    if (!newContent.includes('anthropic_api_key:')) {
      let replacement = `          ${tokenConfig.line}`;
      if (tokenConfig.commentLine) {
        replacement = `          ${tokenConfig.commentLine.trim()}\n${replacement}`;
      }
      result = result.replace(
