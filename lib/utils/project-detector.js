...
async function detectProjectType(projectPath = process.cwd()) {
  ...
  // Prioritize frameworks over generic language detection
  // If we have both Node.js and PHP, check if PHP is actually Laravel
  if (detectedTypes.includes('node') && detectedTypes.includes('php')) {
    ...
    try {
      ...
      if (dependencies['laravel/framework']) {
        // Laravel detected - prioritize PHP over Node.js but keep both
        const nodeIndex = detectedTypes.indexOf('node');
        if (nodeIndex !== -1) {
          detectedTypes.splice(nodeIndex, 1); // Remove node from current position
          detectedTypes.unshift('php'); // Put php first
          detectedTypes.push('node'); // Add node back as secondary
        }
      }
    } catch (error) {
      // Continue with original detection order
    }
  }
...
async function enhancePHPDetails(details, projectPath) {
  ...
  if (await fs.pathExists(composerPath)) {
    ...
    if (dependencies['laravel/framework']) {
      ...
      // Use Laravel framework version if no project version
      if (details.version === 'unknown' && dependencies['laravel/framework']) {
        const laravelVersion = dependencies['laravel/framework'].replace(/[^\d\.]/g, '');
        details.version = `Laravel ${laravelVersion}`;
      }
