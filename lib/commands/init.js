...
async function executeSetup(config) {
  ...
  if (config.copyWorkflows) {
    ...
    try {
      ...
      let message = `Workflow files processed (${workflowResult.copied} total) with base_branch: "${workflowResult.baseBranch}" ✓`;

      const statusParts = [];
      if (workflowResult.updated > 0) {
        statusParts.push(`${workflowResult.updated} updated`);
      }
      if (workflowResult.merged > 0) {
        statusParts.push(`${workflowResult.merged} merged with user changes`);
      }
      if (workflowResult.removed > 0) {
        statusParts.push(`${workflowResult.removed} old files removed`);
      }

      if (statusParts.length > 0) {
        message = `Workflow files updated (${statusParts.join(', ')}) with base_branch: "${workflowResult.baseBranch}" ✓`;
      }
