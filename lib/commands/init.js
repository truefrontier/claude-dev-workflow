...
async function initCommand(options) {
  ...
  try {
    ...
    if (existingInstall.hasWorkflows) {
      await showUpdateSuccessMessage(projectDetails, existingInstall);
    } else {
...
async function checkExistingInstallation() {
  ...
  try {
    ...
    const v2Files = ['stage-specify.yml', 'stage-plan.yml', 'stage-tasks.yml', 'stage-develop.yml'];
...
function getUpdateConfig() {
  return {
    ...
    setupLabels: true,         // Update labels
...
async function interactiveUpdateSetup(existingInstall) {
  ...
  const config = {
    copyWorkflows: true,  // Always update workflows
    setupLabels: true,    // Always update labels
  };

  // Only ask about things that might not be configured
  const additionalQuestions = [];

  // Check if Claude app might need installation
  try {
    execSync('gh api repos/{owner}/{repo}/installation', { stdio: 'pipe' });
    config.installClaudeApp = false;
  } catch {
    additionalQuestions.push({
      type: 'confirm',
      name: 'installClaudeApp',
      message: 'Claude GitHub App not detected. Install it now?',
      default: true
    });
  }
  ...
  try {
    const collaborators = execSync('gh api repos/{owner}/{repo}/collaborators', { stdio: 'pipe' }).toString();
    if (!collaborators.includes('claude-dev-truefrontier')) {
      additionalQuestions.push({
        type: 'confirm',
  ...
  try {
    ...
    config.configureSecret = false;
  } catch {
    additionalQuestions.push({
      type: 'confirm',
      name: 'configureSecret',
      message: 'ANTHROPIC_API_KEY not found. Configure it now?',
      default: true
    });
  }
  ...
  config.createSampleIssue = false;  // Never create sample issue on update

  console.log(chalk.green('\n✓ Update configuration ready'));
...
async function executeSetup(config) {
  ...
  if (config.copyWorkflows) {
    ...
    try {
      ...
      const result = await copyWorkflowFiles();
      let message = `Workflow files copied (${result.copied}) ✓`;
      if (result.removed > 0) {
        message = `Workflow files updated (${result.copied} copied, ${result.removed} old files removed) ✓`;
      }
...
async function showUpdateSuccessMessage(projectDetails, existingInstall) {
  const commands = generateCommands(projectDetails);

  console.log(chalk.green.bold('\n✅ Claude Dev Workflow updated successfully!'));

  console.log(chalk.blue('\n🔄 Update Summary:'));
  console.log(`   Previous: v${existingInstall.version} (${existingInstall.workflowCount} files)`);
  console.log('   Current: v2 (5 files)');

  if (existingInstall.hasV1Files) {
    console.log(chalk.yellow('   • Old v1 workflows removed and replaced'));
    console.log('   • New v2 spec-kit methodology implemented');
  }
  ...
  console.log(chalk.blue('\n🚀 Ready to Use:'));
  console.log('• Your existing issues can now use the updated workflows');
  console.log('• Comment @claude-dev-truefrontier on any issue to start');
  console.log('• New v2 system: specify → plan → tasks → develop');

  console.log(chalk.green.bold('\n🎉 All set! Your workflows are now up to date.'));
}
